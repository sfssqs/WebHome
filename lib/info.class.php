...
class info extends base{
	...
	public $table_co='info_co';		            //本系统的内容表
	public $table_lm='info_lm';		            //本系统的分类表
	public $sy_id=6;                            //本系统的seo信息的记录id(同时也是多图、相关信息、相关文件、相关视频的系统区分id，因为整站的多图、相关信息、相关文件、相关视频都是只保存在4个表中)
	...
	public function getcoarr($psize){
		...
		//信息列表的sql语句
		$sql='select `id`,`lm`,`title'.$this->lang.'`,`link_url`,`apname`,`wtime`,`fil_sl`,`f_body'.$this->lang.'` from '.$this->table_co .' where pass=1 '.$sq.' order by ding desc,px desc,id desc';		
		//实例化分页对象
	...
	public function showlm_1_1($pname,$sub=0){
		...
		if(isset($arr[1][$fid])){
			foreach($arr[1][$fid] as $k=>$v){
				...
				$show_html .= '<li><a href="'.$turl.'" '.$targ.' '.$tcur.'>'.$title_lm.'</a></li>'."\n";
			}
		}
		echo $show_html;
	}
	...
	*/
	public function showlist_1($pname,$psize=12,$sub=0,$f_sub=0,$last=0){
		...
		foreach($arr as $k=>$v){
			...
			$show_html .= '<li '.$tlast.'><span>'.date('Y-m-d',$v['wtime']).'</span><a href="'.$turl.'" '.$targ.'>'.$title.'</a></li>'."\n";
		}
		echo $show_html;
	}
