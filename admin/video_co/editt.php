...
$sql='update `'.$conf['sy']['table_co'].'` set `lm`='.$lm.',`list_lm`="'.$list_lm.'",`title`="'.$title.'",`title_en`="'.$title_en.'",`keyword`="'.$keyword.'",`link_url`="'.$link_url.'",`f_body`="'.$f_body.'",`z_body`="'.$z_body.'",`img_sl`="'.$img_sl.'",`vid_sl`="'.$vid_sl.'",`ym_tit`="'.$ym_tit.'",`ym_key`="'.$ym_key.'",`ym_des`="'.$ym_des.'",`px`='.$px.' where `id`='.$id.'';
$db->execute($sql);
