		...
		function createView(result) {
			...
			for (var i = 0, len = fileList.length; i < len; i++) {
				...
				if(data.is_photo){//如果是图片
					var _span=K('<span class="xl_span" onclick="getstatus(this,\''+K.formatUrl(result.current_url + data.filename, 'absolute')+'\')" data-url="'+K.formatUrl(result.current_url + data.filename, 'absolute')+'" style="position:absolute;display:none;width:102px;background:#0690d2;color:#FFF;text-align:center; cursor: pointer;line-height:20px;bottom:-3px;left:0;">删除</span>');
					div.append(_span);
				}
			}
/*			//以下代码需要jquery库的支持，所以我用js代替了
			$(".xl_span").click(function(){//这里的.xl_span对应着刚才插入的删除按钮上的class
				var $this=K(this);
				if(!confirm('确定删除吗 ?')) {//提示，如果点击取消则直接退出
					return false;
				}
				$.post('../kd_html/php/file_manager_json.php',{action:"delete",url:$this.attr("data-url")},function(res){//jquery的post，action为指定的配合后端用，url是获取刚才存在删除按钮上的图片路径，你完全可以用别的来写，因为用不好kindeditor的js库才用的jquery
...
function getstatus(obj,url){
	...
	if (ajaxHttp!=null) {
		...
		ajaxHttp.open("POST","../kd_html/php/file_manager_json.php?r="+Math.random(),true);
		ajaxHttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		ajaxHttp.send("action=delete&url="+url);
	}
} 