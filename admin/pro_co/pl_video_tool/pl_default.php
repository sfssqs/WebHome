...
<input type="button" name="Submit1" value="添加视频" class="btn" onclick="parent.tanchuchuang('pl_video_tool/pl_add.php?pl_id=<?php echo $pl_id?>',620,250);"> 
</div>
<?php
//把图片列出来
if (($pl_id!=''&&checknum($pl_id))||(isset($_SESSION[$pl_sesname])&&$_SESSION[$pl_sesname]!=''&&checknum($_SESSION[$pl_sesname]))){
	if ($pl_id!=''){
		$sql='select id,pl_id,title,vid_sl,px,wtime,read_num from '.$pl_table.' where sy_id='.$pl_sy_id.' and pl_id='.$pl_id.' order by px asc,id asc';
	}else{
		$sql='select id,pl_id,title,vid_sl,px,wtime,read_num from '.$pl_table.' where sy_id='.$pl_sy_id.' and pl_id='.$_SESSION[$pl_sesname].' order by px asc,id asc';
	}
	$rss=$db->getrss($sql);
	if ($rss){
?>
<form id="form1" name="form1" action="pl_make.php?act=edit_pl&pl_id=<?php echo $pl_id?>" method="post" >
    ...
    <tr class="title">
      ...
      <tr class="tdbg">
        ...
        <td>
		<?php 
		echo $row['title'].(($row['vid_sl']!='')?'<a href="../../../'.$row['vid_sl'].'"  target=_blank><img src="../../images/rar.gif" border="0"/></a>':'');
		?>
        </td>
        <td align="center"><?php echo $row['read_num']?></td>
        <td align="center">
        <a href="javascript:;" onclick="parent.tanchuchuang('pl_video_tool/pl_edit.php?id=<?php echo $row['id']?>&pl_id=<?php echo $row['pl_id']?>',620,250);">修改</a> | <a href="pl_make.php?id=<?php echo $row['id']?>&pl_id=<?php echo $pl_id?>&act=del"  onClick="return confirm(\'确定要删除该数据吗?\')" >删除</a>
        </td>
      </tr>
        <?php
            $a++;
