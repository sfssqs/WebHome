        ...
        <table border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td <?php if ($conf['sy']['need_lm']==false){echo ' style="display:none;"';}?>>
                <select name="lm" id="lm" onchange="location='default.php?lm='+this.value+'&lx=<?php echo $lx?>&zt_val=<?php echo $zt_val?>&keyword=<?php echo $keyword?>'">
      		...
      		<td <?php if ($conf['sy']['need_lx']==false){echo ' style="display:none;"';}?>>
                <select name="lx" id="lx" onchange="location='default.php?lx='+this.value+'&lm=<?php echo $lm?>&zt_val=<?php echo $zt_val?>&keyword=<?php echo $keyword?>'">
			...
			?>
            <td>
                <select name="zt_val" id="zt_val" onchange="location='default.php?zt_val='+this.value+'&lm=<?php echo $lm?>&lx=<?php echo $lx?>&keyword=<?php echo $keyword?>'">
...
<?php
$sql='select a.`id`,a.`lm`,a.`title`,a.`ding`,a.`tuijian`,a.`hot`,a.`pass`,a.`px`,b.`title_lm` from `'.$conf['sy']['table_co'].'` a left join `'.$conf['sy']['table_lx'].'` b on a.`lx`=b.`id_lm` where 1=1 '.$sq.' order by a.ding desc,a.`px` desc,a.`id` desc';
...
?>
    <tr class="tdbg">
		...
		<?php 
		$del_a=' | <a href="make.php?id='.$row['id'].'&ac=del"  onClick="return confirm(\'确定要删除该数据吗?\')" >删除</a>';
