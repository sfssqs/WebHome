...
if($ac==''){
	if($img_sl==''){
		echo'<script>location="up.php?frameid='.$frameid.'&kuang='.$kuang.'&s_pic='.$s_pic.'&s_typ='.$s_typ.'&s_wid='.$s_wid.'&s_hei='.$s_hei.'"</script>';
	}else{
		echo'&nbsp;<a href="../../../'.$img_sl.'" target="_blank"><img src="../../images/img.gif" onmouseover="popImage(this,\'../../'.$img_sl.'\');" onmouseout="hideLayer();"></a> <a href="uploadd.php?frameid='.$frameid.'&kuang='.$kuang.'&img_sl='.$img_sl.'&ac=del&s_pic='.$s_pic.'&s_typ='.$s_typ.'&s_wid='.$s_wid.'&s_hei='.$s_hei.'" onclick="return confirm(\'确定要删除文件吗？\')">删除文件,重新上传</a>';
	}
}elseif($ac=='del'){
	if($img_sl!=''){
		delfile($img_sl);
		delfile(getimgj($img_sl,'d'));
	}
