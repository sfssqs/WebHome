...
$sql='update '.$pl_table.' set `title`="'.$title.'",`link_url`="'.$link_url.'",`z_body`="'.$z_body.'",`img_sl`="'.$img_sl.'",`fil_sl`="'.$fil_sl.'",`ym_tit`="'.$ym_tit.'",`ym_key`="'.$ym_key.'",`ym_des`="'.$ym_des.'",`px`='.$px.' where id='.$id.'';
$db->execute($sql);

msg('修改成功','parent.document.getElementById("fra_file").src="pl_file_tool/pl_default.php?pl_id='.$pl_id.'";parent.tanchuCancle()');
?>