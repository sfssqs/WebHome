...
if (($pl_id!=''&&checknum($pl_id))||(isset($_SESSION[$pl_sesname])&&$_SESSION[$pl_sesname]!=''&&checknum($_SESSION[$pl_sesname]))){
	if ($pl_id!=''){
		$sql='select id,pl_id,title,img_sl,px,wtime,read_num from '.$pl_table.' where sy_id='.$pl_sy_id.' and pl_id='.$pl_id.' order by px asc,id asc';
	}else{
		$sql='select id,pl_id,title,img_sl,px,wtime,read_num from '.$pl_table.' where sy_id='.$pl_sy_id.' and pl_id='.$_SESSION[$pl_sesname].' order by px asc,id asc';
	}
	$rss=$db->getrss($sql);
	if ($rss){
?>
