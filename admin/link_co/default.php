...
<?php
$sql='select a.`id`,a.`lm`,a.`title`,a.`img_sl`,a.`ding`,a.`tuijian`,a.`hot`,a.`pass`,a.`read_num`,a.`px`,b.`title_lm`,b.`add_xx` from `'.$conf['sy']['table_co'].'` a left join `'.$conf['sy']['table_lm'].'` b on a.`lm`=b.`id_lm` where 1=1 '.$sq.' order by a.ding desc,a.`px` desc,a.`id` desc';
...
?>
    <tr class="tdbg">
		...
		echo '<a href="edit.php?id='.$row['id'].'" >'.$row['title'].'</a>'.(($row['img_sl']!='')?'<a href="../../'.$row['img_sl'].'"  target=_blank><img src="../images/img.gif" border="0" onmouseover="popImage(this,\'../../'.$row['img_sl'].'\');" onmouseout="hideLayer();"/></a>':'');
		?>
        </td>
